module Erp
  class User < ApplicationRecord
    ...
    def get_user_group_name
			user_group.present? ? user_group.get_name : ''
		end
    ...
    if Erp::Core.available?('contacts')
        ...
        contact.present? ? contact.get_name : ' '
      end
		end
    ...
    def self.filter(query, params)
      ...
      if global_filter.present?
        user_group_id = global_filter[:user_group_id].present? ? global_filter[:user_group_id] : nil
        query = query.where(user_group_id: user_group_id) if user_group_id.present?
			end
      return query
    end
    ...
    def self.dataselect(keyword='')
      ...
      query = query.limit(10).map{|user| {value: user.id, text: user.get_name}}
    end
    ...
    def self.create_with_omniauth(auth)
			user = User.new ({provider: auth['provider'], uid: auth['uid'], name: auth['info']['name'], email: auth['info']['email'].present? ? auth['info']['email'] : "#{(0...50).map { ('a'..'z').to_a[rand(26)]}.join}@unknown.com", password: 'aA456321@'})
    ...
    def self.permission_options
			{
				'products_comments' => {
					'create' => {'default' => 'yes', 'type' => 'checkbox', 'options' => ['no', 'yes']},
					'delete' => {'default' => 'no', 'type' => 'checkbox', 'options' => ['no', 'yes']}
				},
				'products_ratings' => {
					'create' => {'default' => 'yes', 'type' => 'checkbox', 'options' => ['no', 'yes']},
					'delete' => {'default' => 'no', 'type' => 'checkbox', 'options' => ['no', 'yes']}
